 ...
 */
 ...
 function createTemplateList() {
 	// effettuo il pre load delle librerie
 	preLoad();
...
/********* funzioni test per caricamento template *********/
function preLoad() {
...
function libraryLoad(type, num) {
	for (var i = 1; i < num+1; i++) {
		var libsUrl = 'templates/'+type+i+'/'+type+i+'_libs.json';
...
function selectTml(el) {
	...
	if ( id.match(/tml(\d+)/i) || id.match(/ctml(\d+)/i) ) { // template senza jQuery
		...
		$.getJSON(datiUrl, function(dati) { // se il caricamento ha successo
			...
			Ractive.load(htmlUrl).then( function(Template) {
				// creo l'oggetto ractive
				ractive = new Template({
					el: templateAnchor,
					data: dati
				});
...
function creaTml(type,dati) {
	...
	*******************************************/
	if ( type.match(/ctml(\d+)/i)  || type.match(/jctml(\d+)/i)) {
		...
		creaEditorFullJson( dati, editorAnchor );
